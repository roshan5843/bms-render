...
# combine_shards_rotate.py — R2 VERSION
import json
import os
...
# ← only difference from combine_shards.py
DATE_CODE = os.environ["DATE_CODE"]
LAST_UPDATED = NOW_IST.strftime("%Y-%m-%d %H:%M IST")
R2_BASE = f"advance/{DATE_CODE}"

print(f"📁 R2 base path: r2://{R2_BASE}")
print(f"⏱ Last updated: {LAST_UPDATED}")

# Everything else is IDENTICAL to combine_shards.py above
# (same normalize_row, dedupe, load shards, build summary, upload)
# Just paste the rest of combine_shards.py here unchanged
...
r2_upload_json(f"{R2_BASE}/finaldetailed.json",
...
summary = {}
for r in final_rows:
    movie, city, state, venue, chain = r["movie"], r["city"], r["state"], r["venue"], r["chain"]
    total, sold, gross = r["totalSeats"], r["sold"], r["gross"]
    occ = (sold / total * 100) if total else 0

    if movie not in summary:
        summary[movie] = {"shows": 0, "gross": 0.0, "sold": 0, "totalSeats": 0,
    ...
    ck = (city, state)
    if ck not in m["details"]:
        m["details"][ck] = {"city": city, "state": state, "venues": set(), "shows": 0,
                            "gross": 0.0, "sold": 0, "totalSeats": 0, "fastfilling": 0, "housefull": 0}
    d = m["details"][ck]
    d["venues"].add(venue)
    d["shows"] += 1
    d["gross"] += gross
    ...
    if chain not in m["Chain_details"]:
        m["Chain_details"][chain] = {"chain": chain, "venues": set(), "shows": 0,
                                     "gross": 0.0, "sold": 0, "totalSeats": 0, "fastfilling": 0, "housefull": 0}
    c = m["Chain_details"][chain]
    c["venues"].add(venue)
    c["shows"] += 1
...
for movie, m in summary.items():
    final_summary[movie] = {
        "shows": m["shows"], "gross": round(m["gross"], 2), "sold": m["sold"],
        "totalSeats": m["totalSeats"], "venues": len(m["venues"]), "cities": len(m["cities"]),
        "fastfilling": m["fastfilling"], "housefull": m["housefull"],
        "occupancy": round((m["sold"]/m["totalSeats"])*100, 2) if m["totalSeats"] else 0.0,
        "details": [
            {**{k: v for k, v in d.items() if k != "venues"}, "venues": len(d["venues"]),
             "occupancy": round((d["sold"]/d["totalSeats"])*100, 2) if d["totalSeats"] else 0.0}
            for d in m["details"].values()
        ],
        "Chain_details": [
            {**{k: v for k, v in c.items() if k != "venues"}, "venues": len(c["venues"]),
             "occupancy": round((c["sold"]/c["totalSeats"])*100, 2) if c["totalSeats"] else 0.0}
            for c in m["Chain_details"].values()
        ]
    }

r2_upload_json(f"{R2_BASE}/finalsummary.json",
