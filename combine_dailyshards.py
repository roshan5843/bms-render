...
print(f"📦 Reading shards from: {SHARD_BASE}")
print(f"⏱ Last updated: {LAST_UPDATED}")
...
# =====================================================
# 🔥 LOAD FROM SHARD ARTIFACTS (UPDATED)
# =====================================================
all_rows = []

if not os.path.exists(SHARD_BASE):
    print("❌ No shards folder found!")
    exit(1)

for shard_name in os.listdir(SHARD_BASE):
    shard_path = os.path.join(SHARD_BASE, shard_name)

    if not os.path.isdir(shard_path):
        continue

    print(f"📂 Reading {shard_name}")

    for file in os.listdir(shard_path):
        if file.startswith("detailed") and file.endswith(".json"):
