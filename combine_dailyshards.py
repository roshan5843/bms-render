...
def get_period(show_time_str):
    """
    Classify a show's start time into one of four slots.
    ...
    Expects the same '%I:%M %p' format stored in r["time"] by every shard.
    Returns None if parsing fails so callers can safely skip.
    """
    try:
...
def _empty_periods():
    """Zero-filled accumulator for one movie's period breakdown."""
    return {
...
for r in final_rows:
    ...
    # ── NEW: bucket this show into the right time period ─────────────────
    ...
    # ── city/state detail (unchanged) ────────────────────────────────────
    ck = (city, state)
    if ck not in m["details"]:
        m["details"][ck] = {
            "city": city, "state": state, "venues": set(),
            "shows": 0, "gross": 0.0, "sold": 0, "totalSeats": 0,
            "fastfilling": 0, "housefull": 0,
        }
    d = m["details"][ck]
...
for movie, m in summary.items():
    # ── NEW: compute per-period occupancy from accumulated totals ─────────
    periods_out = {
        slot: {
            "shows":      pd["shows"],
            "sold":       pd["sold"],
